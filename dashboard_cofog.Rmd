...
renderPlot({
  ...
  dados_cofog_completo %>%
    ...
    geom_jitter( show.legend = FALSE, size=4, height = 0, width = ifelse(dados_cofog_completo$cluster==1,0.2,0.1))+  
    ...
    geom_jitter(data= dados_filtrados,  size = 4, show.legend = FALSE, height = 0, width = ifelse(dados_filtrados$cluster==1,0.2,0.1)) +
    #geom_text(data = dados_alerta, aes(x=1.2, y= min_total,  label="<---Mudança de escala"), color = "red") +
...
renderPlotly({
  ...
  dados_cofog_completo <-
    ...
    rename(descricao_cofog =  descricao_cofog.x,
           descricao_cofog_pai = descricao_cofog.y)%>%
    mutate(cofog_path = paste(descricao_cofog_pai))
  ...
  dados_cofog_completo %>%
    ...
    filter(total>0) %>%
  ggplot(aes(x=str_wrap(factor(cofog_path) ,20), y= total/10^6))+
    geom_jitter(aes(color =  str_wrap(factor(descricao_elemento_despesa) ,20)))+  
    #geom_line(aes())
    geom_jitter(data= dados_filtrados, aes(color= str_wrap(factor(descricao_elemento_despesa) ,20)), alpha= 0.5) +
    scale_y_log10(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
    scale_color_viridis(discrete=TRUE) +
    theme_light()+ 
    theme(
