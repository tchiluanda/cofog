...
get_dados_economicos<- function(a_ano){
  ...
  #   mutate(codigo_cofog_pai = ifelse(str_length(codigo_cofog)==3,"7",str_sub(codigo_cofog,1,3)) ) %>%
  #   mutate(funcao_economica = str_squish(funcao_economica)) %>%
  #   mutate(descricao_cofog = str_squish(descricao_cofog)) %>%
  #   separate(funcao_economica, into = c("gfs_pai","gfs_filho"), sep = "-") %>%
  #   mutate(funcao_economica = gfs_filho) %>%
  #   separate(descricao_cofog, into = c("cofog_pai","cofog_filho"), sep = "-") %>%
  #   mutate(descricao_cofog_pai = cofog_pai) %>%
  #   mutate(descricao_cofog_filho = cofog_filho) %>%
  #   mutate(funcao_economica = str_trim(funcao_economica, side = "both")) %>%
  #   mutate(descricao_cofog_filho = str_trim(descricao_cofog_filho, side = "both")) %>%
  #   mutate(descricao_cofog_pai = str_trim(descricao_cofog_pai, side = "both")) %>%
  #   select(codigo_cofog,
  #          descricao_cofog_filho,
  #          descricao_cofog_pai,
  #          funcao_economica,
  #          valor,
  #          codigo_cofog_pai)%>%
  #   mutate(descricao_cofog_pai = case_when(
  #     descricao_cofog_pai == "Despesa total3/" ~  "Despesa total" ,
  #     descricao_cofog_pai == "Transações da dívida pública4/" ~ "Transações da dívida pública",
  #     descricao_cofog_pai == "Educação infantil e ensino fundamental I5/"~ "Educação infantil e ensino fundamental I",
  #     descricao_cofog_pai == "Sobreviventes" ~"Pensionistas",
  #     TRUE ~descricao_cofog_pai
  #   )) %>%
  #   group_by(codigo_cofog_pai,
  #            codigo_cofog,
  #            descricao_cofog_pai,
  #            descricao_cofog_filho,
  #            funcao_economica) %>%
...
renderSankeyNetwork({
  ...
  sankeyNetwork(Links = links, Nodes = nodes, Source = "source",
                ...
                units = "", fontSize = 12, nodeWidth = 30, nodePadding =5, NodeGroup = "descricao_cofog_pai")
...
renderPlot({
  ...
  #dados_economicos$descricao_cofog <- dados_economicos$descricao_cofog_pai 
  
  #dados_classificacao_funcao<<- dados_economicos

  graph<-
  dados_economicos %>%
  mutate(descricao_cofog_pai =  reorder(descricao_cofog_pai, valor),
         funcao_economica = reorder(funcao_economica, desc(valor))) %>% 
  ggplot(aes(x=descricao_cofog_pai,valor, fill= funcao_economica))+
    geom_col()+
  ...
  if (input$escala){
    graph+
    facet_wrap(str_wrap( funcao_economica,20)~., scales = "free_x")
  } else{
    graph+
    facet_wrap(str_wrap( funcao_economica,20)~.)
  }
...
renderPlot({
  ...
  filter(codigo_cofog_pai %in% (dados_economicos %>%
                               ...
                               select(codigo_cofog_pai))$codigo_cofog_pai) %>%
  mutate(descricao_cofog_filho = reorder(descricao_cofog_filho, valor),
         funcao_economica = reorder(funcao_economica, desc(valor))) %>% 
    
  ggplot(aes(x=factor(descricao_cofog_filho),valor, fill= funcao_economica))+
    geom_col()+
    scale_fill_viridis(discrete=TRUE) +
    scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE)) +
    theme_light()+ 
