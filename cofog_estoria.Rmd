...
Nessa aba é mostrado como se dispera os gastos nas diversas funções e nas ações de governo. O gráfico é montado levando em consideração que os gastos por ação de governo podem ser reunidos em três grandes grupos de acordo com o valor total anual dispendido. Esses grupos são formados a partir de técnicas estatatisticas podendo variar de tamanho e composição a depender do ano selecionado.

Abaixo uma figura que ilustra esse gráfico.

![](distribuicao_cofog.PNG)

## Como explorar o gráfico de gastos por ação de governo.

Através do menu você pode selecioar o ano que deseja explorar. 
...
O gráfico é incialmente construído excibindo todas as açoes de governo que caem nos grupos 2 e 3. Você pode limpara a qualquer momento a seleção das ações que são destacadas no gráfico usando o botão **Limpar Filtro**
...
## Principais achados do gráfico de gastos por ação de governo

O que se percebe logo que se vê o gráfico é que duas ações de governo no grupo 3 dominam uma grande área do gráfico.  As ações são juros e benefícios previdenciários urbanos. Afigura logo abaixo mostra essa predominância para o ano de 2019 onde houve uma queda proporcional das despesas com juros, conforme já indicado anteriormente.
...
Na classificação econômica a ideia é verificar como o total gasto por cada função é distribuído nas categorias econômicas de finanças publicas. A figura abaixo mostra um exmplo de exploração desse dado.
...
É interesante observar que quando exploramos os valores de investimento há um destaque para **Defesa**. Essa função conforme já visto no gráfico de Sankey tem em geral um volume muito pequeno de despesa quando comparado com as demais funções. Já quando se trata de investimentos percebe-se o seu protagonismo. A seleção dessa função para análise mais detalhada, permite descobrir que a maior parte dos gastos está concentrada em **Defesa Militar**
